...
const buildCDSEQuery = (filters: FiltersState, dataset: Dataset): string => {
    ...
    if (dataset === Dataset.Sentinel1) {
        ...
        if (sentinel1.productTypes.length) {
            const sub = sentinel1.productTypes.map(p => `Attributes/OData.CSC.StringAttribute/any(att:att/Name eq 'productType' and att/OData.CSC.StringAttribute/Value eq '${p}')`).join(" or ");
            parts.push(`(${sub})`);
        }

        if (sentinel1.levels.length) {
            const sub = sentinel1.levels.map(l => `Attributes/OData.CSC.StringAttribute/any(att:att/Name eq 'processingLevel' and att/OData.CSC.StringAttribute/Value eq 'LEVEL_${l}')`).join(" or ");
            parts.push(`(${sub})`);
        }
        ...
        if (sentinel1.polarizations.length) {
            ...
            const sub = variants.map(v => `Attributes/OData.CSC.StringAttribute/any(att:att/Name eq 'polarisationChannels' and att/OData.CSC.StringAttribute/Value eq '${v}')`).join(" or ");
            parts.push(`(${sub})`);
        }
    }
